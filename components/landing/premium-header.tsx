...
export function PremiumHeader({ children, showAuthModal }: PremiumHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/[0.06] bg-[#06070d]/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:gap-4 sm:px-6 sm:py-4">
        <Link href="/" className="flex min-h-[44px] min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="hidden text-xs font-medium text-white/35 sm:block">Student-first networking cards</p>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/dashboard"
            className="hidden rounded-full bg-[#5267d9] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(82,103,217,0.28)] transition-all duration-200 hover:bg-[#6376e0] hover:-translate-y-px sm:inline-flex"
          >
            Create my card
          </Link>
        </div>
