...
export function TemplateTile({ template }: { template: DigiCardTemplate }) {
  return (
    <Link
      href="/create-card"
      className="group subtle-panel block overflow-hidden p-4 transition duration-300 hover:-translate-y-1 hover:shadow-panel"
    >
      <div className={`rounded-[26px] ${template.tone} p-4`}>
        <div className={`h-44 rounded-[22px] bg-gradient-to-br ${template.accent} p-4`}>
          <div className="flex h-full flex-col justify-between rounded-[18px] border border-white/20 bg-white/10 p-4 text-white backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-[0.26em]">
                DigiCard
              </span>
              <span className="rounded-full bg-white/15 px-2 py-1 text-[10px] uppercase tracking-[0.2em]">
                QR
              </span>
            </div>
            <div>
              <p className="text-lg font-semibold">Template Preview</p>
              <p className="mt-1 text-sm text-white/80">{template.name}</p>
            </div>
          </div>
        </div>
      </div>
