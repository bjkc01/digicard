export default function DashboardLoading() {
  return (
    <main className="mx-auto grid max-w-7xl gap-6 px-4 py-4 lg:grid-cols-[280px_minmax(0,1fr)] lg:px-6 lg:py-6">
      {/* Sidebar skeleton */}
      <div className="panel flex h-fit flex-col gap-8 p-6">
        <div className="flex items-center gap-3 border-b border-slate-200 pb-6">
          <div className="h-10 w-10 animate-pulse rounded-xl bg-slate-200" />
          <div className="space-y-2">
            <div className="h-3 w-20 animate-pulse rounded-full bg-slate-200" />
            <div className="h-3 w-28 animate-pulse rounded-full bg-slate-100" />
          </div>
        </div>
        <div className="space-y-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-11 animate-pulse rounded-xl bg-slate-100" />
          ))}
        </div>
      </div>

      <section className="space-y-6">
        {/* Header skeleton */}
        <div className="panel flex items-center justify-between p-5">
          <div className="space-y-2">
            <div className="h-3 w-20 animate-pulse rounded-full bg-slate-200" />
            <div className="h-6 w-48 animate-pulse rounded-2xl bg-slate-200" />
            <div className="h-3 w-64 animate-pulse rounded-full bg-slate-100" />
          </div>
          <div className="h-14 w-40 animate-pulse rounded-2xl bg-slate-100" />
        </div>

        {/* Content skeleton */}
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          <div className="panel p-6">
            <div className="mb-8 h-6 w-40 animate-pulse rounded-2xl bg-slate-200" />
            <div className="grid gap-5 md:grid-cols-2">
              {[1, 2, 3].map((i) => (
                <div key={i} className="subtle-panel h-72 animate-pulse" />
              ))}
            </div>
          </div>
          <div className="space-y-6">
            <div className="panel h-56 animate-pulse" />
            <div className="panel h-52 animate-pulse" />
          </div>
        </div>
      </section>
    </main>
  );
}
